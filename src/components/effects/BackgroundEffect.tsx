...
const BackgroundEffect = memo(({ weatherCode, isDay }: BackgroundEffectProps) => {
  ...
  const getAnimationKey = useMemo(() => {
    ...
    if ([51, 53, 55, 56, 57, 61, 63, 65, 66, 67, 80, 81, 82].includes(weatherCode)) {
      ...
      return isDay ? `rainy-${rainIntensity}` : `night-rainy-${rainIntensity}`;
    }

    // Snow conditions
    if ([71, 73, 75, 77, 85, 86].includes(weatherCode)) {
      const snowIntensity = [71, 85].includes(weatherCode) ? 'light' : 'heavy';

      return isDay ? `snowy-${snowIntensity}` : `night-snowy-${snowIntensity}`;
    }
