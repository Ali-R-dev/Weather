  ...
  return (
    ...
    >
      <div className="flex justify-between items-center max-w-sm min-w-sm mx-auto mb-3">
        <div className="text-xs font-medium text-white/90 uppercase tracking-wide">Day Length</div>
      ...
      <div className="relative mb-5 mt-2 max-w-lg mx-auto" style={{ minHeight: '3.5rem' }}>
        ...
        >
          {isDay ? (
            <span
              className={
                styles.sunPulse +
                ' flex items-center justify-center w-9 h-9 rounded-full bg-gradient-to-br from-yellow-300 via-yellow-200 to-yellow-400 border-2 border-yellow-400 shadow-lg'
              }
            ...
            <span
              className={
                styles.moonPulse +
                ' flex items-center justify-center w-9 h-9 rounded-full bg-gradient-to-br from-indigo-300 via-indigo-400 to-blue-400 border-2 border-indigo-300 shadow-lg'
              }
              aria-label="Current moon position"
              role="img"
            >
        ...
        {/* Sunrise icon */}
        <div
          className="absolute left-0 top-1/2 flex flex-col items-center z-30"
          style={{ transform: 'translateY(-135%)' }}
        >
          <SunIcon className="w-6 h-6 text-yellow-200 drop-shadow" type="sun" />
          <span className="text-xs text-yellow-100 mt-1 font-semibold tracking-wide">
            {formatTimeWithFormat(sunrise, timeFormat)}
          </span>
        </div>
        {/* Sunset icon */}
        <div
          className="absolute right-0 top-1/2 flex flex-col items-center z-30"
          style={{ transform: 'translateY(-135%)' }}
        >
          <SunIcon className="w-6 h-6 text-indigo-200 drop-shadow rotate-180" type="sun" />
          <span className="text-xs text-indigo-100 mt-1 font-semibold tracking-wide">
